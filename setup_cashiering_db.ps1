# setup_cashiering_final.ps1
# Script robusto para configurar Cashiering en PMS Zafiro
...
function Write-File {
    ...
    Write-Host "✅ Archivo creado/actualizado: $Path" -ForegroundColor Green
}

Write-Host "🚀 Iniciando configuración FINAL de Cashiering..." -ForegroundColor Cyan
...
# ---------------------------------------------------------
# 4. APPLICATION LAYER (DTOs & Logic)
# ---------------------------------------------------------
...
public class CashierService
{
    ...
    public async Task<CashierShiftDto> OpenShiftAsync(string userId, decimal startingAmount)
    {
        ...
        if (existing != null) throw new InvalidOperationException(""Ya tienes una caja abierta."");
    ...
    {
        var shift = await _repository.GetOpenShiftByUserIdAsync(userId);
        if (shift == null) throw new InvalidOperationException(""No tienes una caja abierta."");
...
public class CashierController : ControllerBase
{
    ...
    {
        var status = await _service.GetStatusAsync(""user1""); // TODO: Auth real
...
# 7. MIGRACIONES
# ---------------------------------------------------------
Write-Host "📦 Gestionando migraciones..." -ForegroundColor Yellow
dotnet ef migrations add AddCashieringV3 -p src/Infrastructure -s src/API
if ($LASTEXITCODE -eq 0) {
    dotnet ef database update -p src/Infrastructure -s src/API
    Write-Host "🎉 Base de datos actualizada correctamente!" -ForegroundColor Green
} else {
    Write-Host "⚠️ Error al crear migración. Verifica si ya existe o hay errores de compilación." -ForegroundColor Red
}